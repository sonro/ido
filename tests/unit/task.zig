...
fn checkTaskFmt(expected: []const u8, task: Task) !void {
    var buf = [_]u8{0} ** 100;
    const formatted = try std.fmt.bufPrint(&buf, "{f}", .{task});
